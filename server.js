...
async function initDb() {
  ...
  if (!p) return;
  try {
    // Base table (includes client_key)
    await p.query(`
    ...
    await p.query(`ALTER TABLE search_events ADD COLUMN IF NOT EXISTS client_key TEXT;`);
    await p.query(`CREATE INDEX IF NOT EXISTS idx_search_events_ts ON search_events (ts DESC);`);
    await p.query(`CREATE INDEX IF NOT EXISTS idx_search_events_query ON search_events (query);`);
    await p.query(`CREATE INDEX IF NOT EXISTS idx_search_events_dest ON search_events (dest_url);`);
    await p.query(`CREATE INDEX IF NOT EXISTS idx_search_events_client_ts ON search_events (client_key, ts DESC);`);
...
/* =========================================================
   Bot filter & per-user/day rate-limit helpers
========================================================= */
const botUA = /(bot|crawler|spider|bingpreview|petalbot|slurp|facebookexternalhit|skypeuripreview|whatsapp|telegrambot|discordbot|headlesschrome|puppeteer|playwright|phantom|uptimerobot|monitoring)/i;

function makeClientKey(req, body) {
  // Prefer explicit client id from payload if present (recommended from frontend)
  if (body && typeof body.cid === 'string' && body.cid.length > 0) {
    return createHash('sha256').update('cid:' + body.cid).digest('hex');
  }
  const ua = (req.header('User-Agent') || '').slice(0, 512);
  const ip = String((req.headers['x-forwarded-for'] || req.socket.remoteAddress || '')).split(',')[0].trim();
  return createHash('sha256').update(`ip:${ip}|ua:${ua}`).digest('hex');
}

const DAILY_LIMIT = parseInt(process.env.DAILY_LIMIT || '20', 10);

/* =========================================================
   Rate limits (network-level & export)
========================================================= */
const ingestNetworkLimiter = rateLimit({ windowMs: 60 * 1000, max: 120 }); // per-IP per-minute (coarse)
...
/* =========================================================
   Ingest endpoint
   - Bot filter: skips DB write, returns 204
   - Per-user/day cap: 20 logs/day (skips DB write after cap, returns 204)
   - Auth: lightweight write token via X-Write-Key header or body.writeKey
========================================================= */
app.post('/v1/log-search', ingestNetworkLimiter, async (req, res) => {
  const body = req.body || {};
  ...
  const ua = (req.header('User-Agent') || '').slice(0, 1024);
  if (botUA.test(ua)) {
    // Silently drop bots (no DB write), keep UX smooth
  ...
  const clientKey = makeClientKey(req, body);
  ...
  try {
    ...
    if (rows[0].n >= DAILY_LIMIT) {
      return res.status(204).end(); // over quota: do nothing, succeed for client
    }
  } catch (e) {
    console.error('Rate check failed:', e.message);
    // Even if rate check fails, we can choose to insert or not. Be conservative: skip write.
    return res.status(204).end();
  }
  ...
  try {
    await p.query(
      `INSERT INTO search_events
       (query, selected_text, label, dest_url, method, rank, matched_synonyms, source_path, user_agent, referrer, score, client_key)
    ...
    return res.status(204).end(); // good for sendBeacon
  } catch (err) {
...
app.get('/export.csv', exportLimiter, requireBasicAuth, async (req, res) => {
  ...
  const headers = [
    ...
    'matched_synonyms','source_path','user_agent','referrer','score','client_key'
  ];
...
app.get('/stats', exportLimiter, requireBasicAuth, async (req, res) => {
  ...
  days = Math.min(Math.max(days, 1), 90); // clamp
